...
export function buildContextPack(repoPath: string, opts: { budget?: number; now?: number } = {}): ContextPack {
  ...
  const header =
`SYSTEM / INSTRUCTIONS FOR THE AI ASSISTANT — read this first.

You have just been given a complete CONTEXT PACK of a software project named "${repoName}".
It contains the repo's full structure plus its highest-signal files (FULL = verbatim
content; OUTLINE = exact function/class SIGNATURES with bodies elided to save space;
secrets shown as «REDACTED:…»). Treat it as the ground truth for this codebase.

YOUR TASK, right now, before anything else:
1. Read the whole pack.
2. Reply with a 3–4 sentence summary of what this project does and how it's architected.
3. List the 3 areas most likely to hold bugs or need refactoring (cite file paths).
4. Then ask: "What would you like help with?" and wait for me.

RULES: Ground every answer in the files below. If something I ask about isn't in the
pack, say "that file isn't in the pack" instead of guessing — do not invent code.

═══════════════════════════════════════════════════════════════════════════════

# 📦 AI Context Pack — ${repoName}

> Built by Mneme X-Ray: **prioritized** by real signals (not a raw dump),
> **secret-redacted**, and **token-budgeted** to fit your model. ~${used.toLocaleString()} tokens.
>
> **Coverage:** ${coverage}

## 💡 After the AI summarises, try asking:
- "Explain how <feature> works and which files it lives in."
- "Find the riskiest code here and suggest concrete fixes."
- "I want to add <feature> — exactly which files do I change, and how?"
- "Review <path> for bugs, security and edge cases."
- "Draw the architecture / data flow of this project."
