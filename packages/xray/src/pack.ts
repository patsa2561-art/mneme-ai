...
export function buildContextPack(repoPath: string, opts: { budget?: number; now?: number } = {}): ContextPack {
  ...
  for (let i = 0; i < scored.length; i++) {
    if (used >= budget) { omitted = scored.length - i; break; }
    const f = scored[i];
    ...
    if (used + cost > budget && used > 0) { omitted = scored.length - i; break; }
    used += cost;
...
> Generated by Mneme X-Ray. **Prioritized** (signal, not a raw dump), **secret-redacted**, **token-budgeted** to fit your model's window. ~${used.toLocaleString()} tokens${omitted ? ` · ${omitted} less-important files omitted to fit the budget` : ""}.
...
## 🗂 Structure
\`\`\`
${tree(files.map((f) => f.rel))}
\`\`\`

## 📄 Files (prioritized; FULL for central files, OUTLINE = signatures-only for the rest; secrets redacted)
`;
