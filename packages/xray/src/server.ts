...
/** A real, deterministic example report (computed once, memoized) — the auto-animating hero on /suite. */
let _featured: { grade: string; score: number; gc: string; fns: number; tables: number; endpoints: number; risk: Array<{ ic: string; name: string; factor: string }>; authz: string; tests: string } | null = null;
function featuredReport() {
  ...
  const risk = riskHotspots.riskHotspots(files, { top: 3, graph: g }); const rsum = riskHotspots.riskSummary(risk);
  ...
  const GC: Record<string, string> = { A: "#22c55e", B: "#84cc16", C: "#eab308", D: "#f97316", F: "#ef4444" };
  _featured = {
    grade, score, gc: GC[grade], fns: byType("function"), tables: byType("db_table"), endpoints: byType("api_endpoint"),
    risk: risk.map((h) => ({ ic: h.band === "CRITICAL" ? "🔴" : h.band === "HIGH" ? "🟠" : "🟡", name: h.name, factor: (h.factors[0] || "").slice(0, 60) })),
    authz: authz.clear ? "✓ no unguarded sensitive-write path" : `🔴 ${authz.count} unguarded write → ${authz.worstTables.join(", ")}`,
    tests: `keystones ${tg.coveredKeystones}/${tg.totalKeystones} guarded${tg.uncoveredKeystones.length ? ` · ⚠️ untested: ${tg.uncoveredKeystones.slice(0, 2).map((k) => k.node.name).join(", ")}` : ""}`,
  };
  return _featured;
}
...
function suiteLandingHtml(): string {
  const fr = featuredReport();
  let d = -1; const rev = (extra = "") => `class="reveal${extra ? " " + extra : ""}" style="animation-delay:${(d += 1) * 0.12}s"`;
  const heroRisk = fr.risk.map((r) => `<div class="hr" ${rev()}><span>${r.ic}</span> <b>${xesc(r.name)}</b> <span class="hrf">${xesc(r.factor)}</span></div>`).join("");
  const hero =
    `<div class="hero"><div class="hbar"><span class="hd r"></span><span class="hd y"></span><span class="hd g"></span><span class="ht">mneme review · live example</span></div>` +
    `<div class="hbody">` +
    `<div class="hgrade" ${rev("pop")} style="background:${fr.gc}">${fr.grade}</div>` +
    `<div class="hmeta"><div ${rev()} class="hcmd">$ npx mneme review</div>` +
    `<div ${rev()} class="hg">🕸 ${fr.fns} functions · ${fr.tables} tables · ${fr.endpoints} endpoints — linked across layers</div>` +
    `<div class="hbarw" ${rev()}><div class="hbarf" style="width:${fr.score}%;background:${fr.gc};animation-delay:${(d + 1) * 0.12}s"></div></div>` +
    `<div ${rev()} class="hscore">${fr.score}/100</div></div></div>` +
    `<div class="hsec" ${rev()}>🎯 <b>RISK HOTSPOTS</b></div>${heroRisk}` +
    `<div class="hsec" ${rev()}>🔒 <b>AUTHZ</b> &nbsp;${xesc(fr.authz)}</div>` +
    `<div class="hsec" ${rev()}>🧪 <b>TESTS</b> &nbsp;${xesc(fr.tests)}</div>` +
    `<div class="hfoot" ${rev()}>↑ a real report, computed deterministically — <b>no LLM guessed a thing</b>. Now run it on your repo ↓</div>` +
    `</div></div>`;
