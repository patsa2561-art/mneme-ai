...
function personaLandingHtml(): string {
  ...
  const js = [
    ...
    "function render(d){if(!d.personas||!d.personas.length){st.className='err';st.textContent='No commits found to analyze.';return;}CURREPO=d.repo||'';rep.innerHTML='<h2 class=rt>'+esc(d.repo)+' — '+d.personas.length+' hero(es)</h2><div class=grid>'+d.personas.map(card).join('')+'</div><p class=foot>🎭 each hero is generated from <b>measured git signals</b> (deterministic — same history, same hero). ⚡ = commit <b>hygiene</b>, not skill or worth. Source was cloned, scanned, and <b>deleted</b>.</p>';rep.style.display='block';rep.scrollIntoView({behavior:'smooth',block:'nearest'});}",
