...
export const SEANCE_TOOLS: MnemeTool[] = [
  {
    ...
    inputSchema: { type: "object", properties: { ref: { type: "string", description: "commit ref / sha / tag" }, months: { type: "number", description: "summon self from N months ago" } } },
    ...
    handler: async (rt, args) => {
      try {
        ...
        let hash = "", ref = "HEAD";
        if (typeof args["ref"] === "string" && args["ref"]) { hash = git(cwd, ["rev-parse", args["ref"] as string]).trim(); ref = args["ref"] as string; }
        ...
        if (!hash) return low("could not resolve that ref.");
        const grep = git(cwd, ["grep", "-nI", "-E", "TODO|FIXME|HACK|XXX", hash, "--", "*.ts", "*.tsx", "*.js", "*.py", "*.go", "*.rs"]);
        ...
        const packet = core.seance.reconstructSeance(commits, hash, { ref, todosThen, now: Math.floor(Date.now() / 1000) });
