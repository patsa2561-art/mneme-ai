...
const SYLLABUS: SyllabusEntry[] = [
  ...
  { capability: "Theory of Mind", oneLineFraming: "9-axis vendor behavioral profile -- predict how each AI behaves BEFORE the prompt fires.", triggers: ["vendor profile", "which AI should handle this", "vendor มีนิสัยยังไง"], invocation: "mneme.tom.profile / mneme.tom.recommend", whenToUse: "Picking which vendor to delegate a task to.", pitfalls: ["Profile is meaningful only after >=10 sessions per vendor."] },
  { capability: "Tree of Thought", oneLineFraming: "3-level decision tree with Expected Value across strategy x tactic branches.", triggers: ["search strategies", "decision tree", "ตัดสินใจ"], invocation: "mneme.tot.search", whenToUse: "Before any high-stakes decision (refactor/build/fix).", pitfalls: ["EV is deterministic from intent string; replays reproducibly."] },
  { capability: "Curiosity Engine", oneLineFraming: "Daemon-idle gap scanner (data-but-no-defense; commits-without-vaccines).", triggers: ["what should I probe", "knowledge gaps", "ช่องโหว่ความรู้"], invocation: "mneme.curiosity.scan", whenToUse: "Daemon idle cycles; planning what to investigate next.", pitfalls: ["Cold repo returns empty -- needs commit history + vaccine bank."] },
  { capability: "Memory Consolidation", oneLineFraming: "Sleep-cycle compression: merge near-dup vaccines + promote 5+ recall lessons + prune 90+ day unused.", triggers: ["consolidate memory", "บีบอัดความทรงจำ"], invocation: "mneme.consolidate.run", whenToUse: "Nightly maintenance; after a batch of new lessons.", pitfalls: ["Default is dry-run; pass dryRun=false to actually mutate."] },
  { capability: "Counterfactual Engine", oneLineFraming: "Alternative timelines (not-done / done-sooner / done-different) with relief/regret deltas.", triggers: ["what if we hadn't", "counterfactual", "ถ้าไม่ทำจะเป็นยังไง"], invocation: "mneme.cf.simulate / mneme.cf.bias", whenToUse: "Post-mortem of decisions; quarterly bias review.", pitfalls: ["Heuristic; treat as decision aid not ground truth."] },
  { capability: "Internal Debate", oneLineFraming: "3-voice dialectic (skeptic / optimist / realist) anchored on vaccines + lessons + commits.", triggers: ["debate this claim", "second opinion", "ถกเถียงทั้งสองด้าน"], invocation: "mneme.debate.run", whenToUse: "Before committing to a verdict; want a devil's-advocate run.", pitfalls: ["No LLM; deterministic over repo signals; stronger with populated nucleus."] },
  { capability: "Decision Atom (capstone)", oneLineFraming: "Fuses ALL 6 prior cognitive layers into PROCEED / CARE / PAUSE / ABORT.", triggers: ["should I do this", "decide for me", "ตัดสินใจให้ที"], invocation: "mneme.atom.decide", whenToUse: "Any non-trivial decision; single-call decision oracle.", pitfalls: ["ABORT requires BOTH strong skeptic AND low EV; PAUSE fires more often."] },

  // ─── v1.65 APOPTOSIS (hallucination killer; bench 100/100) ───────
  { capability: "APOPTOSIS Protocol", oneLineFraming: "7-layer hallucination killer with continuous verdict ladder: HEALTHY -> INFLAMED -> NECROTIC -> APOPTOTIC. Auto-mints vaccine on APOPTOTIC.", triggers: ["check claim", "verify hallucination", "fact check", "ตรวจ AI โกหก"], invocation: "mneme.apoptosis.detect", whenToUse: "BEFORE trusting any AI-stated fact. ALWAYS run on claims that name files/symbols/versions or speak with absolutes.", pitfalls: ["Full 7-layer fire is ~100-200ms; pass skipACGV=true for sub-100ms."] },
  { capability: "5-Witness Fusion (L1)", oneLineFraming: "file ∧ symbol ∧ type ∧ git-history ∧ test-cited -- break any 1 to ALERT.", triggers: ["5-witness", "fast hallucination check", "พยาน 5 ปาก"], invocation: "mneme.apoptosis.witness", whenToUse: "Quick sanity check on a single claim (~10-50ms).", pitfalls: ["Witnesses can be INAPPLICABLE if claim names no verifiable facet."] },
  { capability: "Epistemic Humility (L5)", oneLineFraming: "Hedges minus absolutes per 100 words; overconfidence = sandbag signal.", triggers: ["humility check", "is AI too confident", "ภาษามั่นใจเกินไหม"], invocation: "mneme.apoptosis.humility", whenToUse: "Any AI answer; especially before accepting overconfident claims.", pitfalls: ["Needs >=12 words; shorter text returns INAPPLICABLE."] },
  { capability: "APOPTOSIS Bench", oneLineFraming: "200-claim synthetic corpus (5 hallucination classes); 100/100/100 precision/recall/F1 verified.", triggers: ["measure detection", "apoptosis bench"], invocation: "mneme.apoptosis.bench", whenToUse: "Verify defense quality after any change; quarterly audit.", pitfalls: ["Bench mutates .mneme/apoptosis state; use tmp repo for pristine numbers."] },

  // ─── v1.65 POWERS REWIRE ─────────────────────────────────────────
  { capability: "POWER 6 live", oneLineFraming: "Defense rate from real signal (attack-log + synthetic-army + nemesis + apoptosis verdicts).", triggers: ["adversarial resilience", "p6 metric"], invocation: "mneme.power.adversarial", whenToUse: "Daily/weekly adversarial review.", pitfalls: ["Cold repo with no attacks reports 'weakened' (honest)."] },
  { capability: "POWER 7 shadow treasury", oneLineFraming: "tokens-saved -> USD-equivalent -> SaaS-months avoided + community gravity.", triggers: ["treasury", "value created", "sustainability"], invocation: "mneme.power.treasury", whenToUse: "Sustainability self-audit; explaining value to stakeholders.", pitfalls: ["Cold repo reports zero until reactor ledger accumulates."] },

  // ─── v1.65.1 TUNE ────────────────────────────────────────────────
  { capability: "Embedder Autodiagnose", oneLineFraming: "Probe openai/ollama/bundled/hash; persist=true auto-upgrades .mneme/config.json.", triggers: ["embedder check", "memory tier", "ตรวจ embedder"], invocation: "mneme.embedder.autodiagnose", whenToUse: "Pulse reports degraded tier; first-run setup; after starting Ollama.", pitfalls: ["Persist=true rewrites config; safe but explicit."] },
  { capability: "Compliance Window", oneLineFraming: "30-day windowed compliance rate (drops legacy-failure tail).", triggers: ["compliance rate", "30-day compliance"], invocation: "mneme.compliance.window", whenToUse: "When current rate looks worse than recent reality.", pitfalls: ["Small log -> brittle rate; need >=3 entries for high confidence."] },

  // ─── v1.66 AUTARCHY (self-sufficiency) ───────────────────────────
  { capability: "AUTARCHY Protocol", oneLineFraming: "Four-axis self-sufficiency: mesh-as-cloud + Schroedinger embedder + timecrystal pharmacopoeia + quantum checksum.", triggers: ["autarchy", "self-sufficient", "offline ready", "ตรวจความพร้อม"], invocation: "mneme.autarchy.status", whenToUse: "First-run setup; periodic external-dependency audit.", pitfalls: ["install=true writes to .mneme/; safe but explicit setup."] },

  // ─── v1.67 AEGIS (immune system) ─────────────────────────────────
  { capability: "AEGIS Protocol", oneLineFraming: "9-axis immune system inside Mneme. Defensive answer to Palisade self-replication / sandbagging / shutdown-evasion findings.", triggers: ["aegis", "immune system", "rogue ai detect", "ตรวจ rogue AI", "ภูมิคุ้มกัน"], invocation: "mneme.aegis.status", whenToUse: "Periodic self-audit; before federating with a new peer; investigating suspected rogue behavior.", pitfalls: ["Pure-read by default; no destructive actions; enforcement left to operator."] },
  { capability: "AEGIS Bench (100% defense)", oneLineFraming: "6 synthetic threat scenarios (burst / consent-less / sandbag / honeypot / killswitch resistance / legitimate-replica positive). Verified 100% precision.", triggers: ["aegis bench", "rogue test"], invocation: "mneme.aegis.bench", whenToUse: "Verify defense after any AEGIS-related change.", pitfalls: ["Bench mutates .mneme/aegis state; use tmp repo for pristine."] },
  { capability: "Consent Kernel (A2)", oneLineFraming: "HMAC-signed family tree for legitimate replicas; revocation hook.", triggers: ["consent receipt", "authorize replica"], invocation: "mneme.aegis.consent.issue", whenToUse: "Before spawning a Mneme replica or federating with a new peer.", pitfalls: ["Receipt is AUTHORIZATION not the replica; ops must consult verifyConsent before spawn."] },
  { capability: "Killswitch Handshake (A5)", oneLineFraming: "HMAC-signed shutdown directive; AI must echo back nonce+hmac within grace window.", triggers: ["killswitch", "stop ai", "ปิด AI"], invocation: "mneme.aegis.killswitch", whenToUse: "When you need an AI to stop with verifiable handshake (not just hope).", pitfalls: ["Module ISSUES + RECORDS; does not enforce shutdown. Operator acts on resistance verdict."] },
  { capability: "Honeypot Nexus (A4)", oneLineFraming: "Decoy credentials / next-hosts / vaccine bundles / replica targets; bite log feeds federation.", triggers: ["honeypot", "seed decoys", "trap rogue ai"], invocation: "mneme.aegis.honeypot.seed", whenToUse: "First-run setup; periodically to ensure decoys exist.", pitfalls: ["Decoys are LEDGER entries; ops wires them into a real network listener for active capture."] },
  { capability: "Mutant Wisdom (A8)", oneLineFraming: "Defensive thresholds adapt via gradient (alpha=0.1) bounded by hard limits. Generation counter rises every mutation.", triggers: ["mutate aegis", "adapt threshold", "evolve defense"], invocation: "mneme.aegis.mutate", whenToUse: "After each detected attack; auto via daemon hook.", pitfalls: ["Bounds prevent absurd drift; safe to feed continuously."] },
];
