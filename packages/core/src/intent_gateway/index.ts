...
const CONCEPTS: Concept[] = [
  ...
  { command: "mneme verify", triggers: ["is this true", "is this claim", "is this actually true", "fact check", "verify this claim", "is that correct", "double check this fact", "จริงไหม", "ข้อเท็จจริง", "ถูกต้องไหม"] },
  ...
  { command: "mneme savings", triggers: ["how much have you saved", "token savings", "saved us in tokens", "cost saved", "ประหยัดไปเท่าไหร่", "ประหยัด token", "token ไปเท่าไหร่", "เซฟ token"] },
  ...
  { command: "mneme boot", triggers: ["how do i use mneme", "what can mneme do", "where do i start with mneme", "get started with mneme", "how to start using", "เริ่มยังไง", "mneme ทำอะไรได้", "ใช้ยังไง", "ใช้ mneme ยังไง"] },
  { command: "mneme graph reverse", triggers: ["safe to drop", "what depends on this table", "can i delete this", "remove this endpoint", "drop column impact", "who uses this table", "deletion safety", "ลบตารางนี้ปลอดภัยไหม", "อะไรพึ่งตารางนี้", "ลบได้ไหม", "ใครใช้ตารางนี้"] },
  ...
  { command: "mneme change-gate", triggers: ["change gate", "is this safe to commit", "pre-commit gate", "should i commit this", "gate this change", "safe to merge", "ปลอดภัยที่จะ commit ไหม", "เกตก่อน commit", "commit ได้ไหม"] },
  { command: "mneme bdiff", triggers: ["behavioral diff", "what actually changed", "review the diff by behavior", "did behavior change", "git shows noise", "behavioral version control", "diff เชิงพฤติกรรม", "อะไรเปลี่ยนจริง", "review เฉพาะที่ behavior เปลี่ยน"] },
  { command: "mneme equiv-diff", triggers: ["auto equiv", "did my refactor change behavior", "equiv on the diff", "check all changed functions", "behavior preserved across the diff", "refactor เปลี่ยนพฤติกรรมไหมทั้ง diff", "เช็คทุกฟังก์ชันที่แก้"] },
  { command: "mneme equiv", triggers: ["equivalence receipt", "did the refactor change behavior", "differential test", "prove behavior unchanged", "refactor equivalence", "behavioral equivalence", "พิสูจน์พฤติกรรมไม่เปลี่ยน", "refactor เปลี่ยนพฤติกรรมไหม", "equiv"] },
  { command: "mneme trust-serve", triggers: ["trust gateway", "trust service", "serve the gate", "host the firewall", "trust api", "saas in the ai world", "trust as a service", "run the trust server"] },
  { command: "mneme agent-rep", triggers: ["agent reputation", "who ships durable code", "track record", "survival rate", "whose changes get reverted", "agent durability", "reputation", "ใครเขียนโค้ดอยู่รอด", "track record ของ agent"] },
  { command: "mneme scar-mine", triggers: ["mine scars", "build scar corpus", "learn from our past incidents", "derive scars from history", "bootstrap the vaccine", "ขุดแผลเป็น", "สร้าง corpus แผลเป็น", "เรียนจาก incident เก่า"] },
  { command: "mneme scar-check", triggers: ["scar check", "have we made this mistake before", "vaccinate the change", "past incident this resembles", "negative knowledge", "did this break before", "เคยพลาดแบบนี้ไหม", "บทเรียนแผลเป็น", "ฉีดวัคซีนโค้ด"] },
  { command: "mneme arch-firewall", triggers: ["architectural firewall", "regression firewall", "does this break the architecture", "pre-merge gate", "block if it breaks a contract", "is this safe to merge", "ไฟร์วอลล์สถาปัตยกรรม", "เปลี่ยนนี้พังสถาปัตยกรรมไหม", "gate ก่อน merge"] },
  { command: "mneme mediate", triggers: ["mediate", "fair division", "split fairly", "divorce settlement", "divide assets", "resolve conflict", "envy-free split", "who gets what", "แบ่งให้ยุติธรรม", "ไกล่เกลี่ย", "แบ่งมรดก", "หย่าแบ่งทรัพย์สิน"] },
  { command: "mneme appsec", triggers: ["security audit", "appsec", "is this codebase secure", "security posture", "owasp", "vulnerabilities", "is this safe to ship", "ตรวจความปลอดภัย", "repo นี้ปลอดภัยไหม", "ช่องโหว่"] },
  { command: "mneme seance", triggers: ["why did i choose this", "what was i thinking", "talk to my past self", "decision context", "why was this built this way", "ทำไมตอนนั้นถึงเลือกแบบนี้", "เรียกอดีต", "ย้อนดูการตัดสินใจ"] },
  { command: "mneme certify", triggers: ["certify this", "verify this report", "is this deliverable safe to send", "verified by mneme", "trust certificate", "certify ai output", "sign off on this work", "is this safe to send to the client", "รับรองงาน", "ออกใบรับรอง", "งานนี้ส่งลูกค้าได้ไหม", "certify รายงาน"] },
  { command: "mneme protect", triggers: ["check for hallucination", "is this hallucinated", "is this safe to say", "screen this answer", "verify before answering", "protect against hallucination", "is this answer trustworthy", "gate this claim", "ตรวจ hallucination", "คำตอบนี้เชื่อได้ไหม", "กันมั่ว", "เช็คก่อนตอบ", "คำตอบนี้มั่วไหม"] },
  { command: "mneme statguard", triggers: ["is this stats right", "interpret this p value", "p value means", "p-value interpretation", "confidence interval mean", "statistically significant means", "is this statistically correct", "power analysis interpretation", "check my statistics", "ตีความ p value", "p value แปลว่า", "นัยสำคัญทางสถิติ", "ช่วงความเชื่อมั่นแปลว่า", "สถิตินี้ถูกไหม"] },
  { command: "mneme sdc", triggers: ["which agent is lying", "which agent poisoned", "poisoned memory", "agents disagree which is right", "consensus among agents", "byzantine agents", "error correct the mesh", "find the liar among agents", "agent ไหนโกหก", "agent ไหนวางยา", "ความจำถูกวางยา", "หาคนโกหกในกลุ่ม", "เอเจนต์ขัดแย้งกัน"] },
  { command: "mneme funeral", triggers: ["eulogy", "this project is dead", "this repo is dead", "dead repo", "rest in peace", "rip this repo", "archive ceremony", "write a eulogy", "tombstone", "memorial for", "คำไว้อาลัย", "repo ตายแล้ว", "โปรเจกต์ตายแล้ว", "โปรเจกต์นี้ตาย", "ไว้อาลัย", "ฝังโปรเจกต์"] },
  { command: "mneme injection-paths", triggers: ["sql injection", "command injection", "injection", "ssrf", "path traversal", "is this endpoint injectable", "unsanitized input", "input ไม่ sanitize", "endpoint นี้โดน inject ได้ไหม", "ช่องโหว่ injection"] },
  { command: "mneme exfil-paths", triggers: ["data leak", "exfiltration", "does this endpoint leak", "over-exposed fields", "sensitive data in response", "pii leak", "redaction missing", "ข้อมูลรั่ว", "endpoint นี้รั่วไหม", "ข้อมูลอ่อนไหวหลุด response"] },
  { command: "mneme dead-paths", triggers: ["dead code", "dead paths", "write-only table", "read-only table", "unreachable functions", "is this table read", "architectural dead code", "โค้ดตาย", "table ไหนไม่มีใครอ่าน", "ฟังก์ชันที่ไม่ถูกเรียก"] },
  { command: "mneme contract-map", triggers: ["contract map", "architecture contracts", "rank contracts by age", "what contracts are load bearing", "architecture map", "which rules are sacred", "แผนที่สัญญา", "สัญญาไหนรับน้ำหนัก", "contract ทั้งหมด"] },
  { command: "mneme contract-age", triggers: ["invariant lineage", "contract age", "how old is this rule", "when was this established", "how long has this held", "is this contract load bearing", "อายุสัญญา", "กฎนี้เก่าแค่ไหน", "contract เริ่มเมื่อไหร่"] },
  { command: "mneme regressions", triggers: ["architectural regressions", "what contracts broke", "self diagnose architecture", "what invariants regressed", "incident report", "when did the architecture break", "สัญญาอะไรพัง", "สถาปัตยกรรมพังตอนไหน", "regression report"] },
  { command: "mneme coupling", triggers: ["hidden dependencies", "what changes together", "change coupling", "implicit dependencies", "files secretly coupled", "what else should i change", "dependency ที่ซ่อน", "อะไรเปลี่ยนพร้อมกัน", "ไฟล์ผูกกันลับๆ"] },
  { command: "mneme hotspots", triggers: ["refactor targets", "hotspots", "what should i refactor", "churn and coupling", "most entangled files", "where is the risk concentrated", "ควร refactor ตรงไหน", "ไฟล์ไหนเสี่ยงสุด", "hotspot"] },
  { command: "mneme decay", triggers: ["architectural decay", "is the architecture eroding", "coupling over time", "technical debt trend", "entanglement velocity", "is it getting harder to change", "สถาปัตยกรรมเสื่อมไหม", "coupling โตไหม", "หนี้เทคนิคเทรนด์"] },
  { command: "mneme bisect-invariant", triggers: ["when did this break", "which commit broke the invariant", "who made this reachable", "bisect the architecture", "root cause the regression", "when did the contract break", "commit ไหนทำพัง", "ใครทำให้เข้าถึงได้", "สัญญาพังตอนไหน"] },
  { command: "mneme invariants", triggers: ["architectural invariants", "check my architecture rules", "is the contract upheld", "design by contract", "prove the invariant", "architecture rules", "กฎสถาปัตยกรรม", "พิสูจน์ invariant", "ตรวจกฎสถาปัตยกรรม"] },
  { command: "mneme lock", triggers: ["architecture lock", "did i regress the contract", "lock check", "architecture regression", "contract gate", "lock the architecture", "ล็อกสถาปัตยกรรม", "contract เปลี่ยนไหม", "gate สถาปัตยกรรม"] },
  { command: "mneme review", triggers: ["review this codebase", "review the whole codebase", "review this whole codebase", "full report", "codebase health", "audit the repo", "accountability report", "overall risk", "review my repo", "ตรวจโค้ดทั้งหมด", "review โค้ด", "review ทั้งหมด", "รายงานสุขภาพโค้ด", "review repo"] },
  { command: "mneme graph prove-drop", triggers: ["prove its safe to drop", "prove unsafe to drop", "is it provably safe to delete", "prove this endpoint reaches", "proof not guess", "provable drop safety", "พิสูจน์ว่าลบได้ไหม", "พิสูจน์ไม่ใช่เดา"] },
  { command: "mneme accuracy", triggers: ["how accurate is it", "precision and recall", "measured accuracy", "is this actually correct", "accuracy benchmark", "prove the accuracy", "แม่นยำแค่ไหน", "วัดความแม่นยำ", "precision recall เท่าไหร่"] },
  { command: "mneme logic", triggers: ["is my reasoning sound", "does this conclusion follow", "check this logic", "are these premises consistent", "logical contradiction", "verify the argument", "เหตุผลนี้สมเหตุสมผลไหม", "สรุปนี้ตามมาจริงไหม", "ตรวจตรรกะ", "ข้อขัดแย้งเชิงตรรกะ"] },
  { command: "mneme graph graphql-diff", triggers: ["graphql breaking change", "graphql schema diff", "removed graphql field", "did i break the graphql schema", "schema breaking change", "graphql พังไหม", "schema graphql เปลี่ยน"] },
  { command: "mneme graph api-diff", triggers: ["breaking change", "did i break the api", "removed endpoint", "api surface diff", "is this a breaking change", "what api changed", "ทำ api พังไหม", "ลบ endpoint", "breaking change ไหม"] },
  { command: "mneme services", triggers: ["cross-service", "which service calls which", "inter-service dependencies", "microservice contracts", "who calls this api across services", "broken api contract", "service ไหนเรียก service ไหน", "ใครเรียก api นี้", "contract ข้าม service"] },
  { command: "mneme onboard", triggers: ["how does this codebase work", "where do i start reading", "onboard me", "explain the architecture", "give me a tour", "understand this repo", "main flows", "โค้ดนี้ทำงานยังไง", "เริ่มอ่านตรงไหน", "อธิบายสถาปัตยกรรม", "พาทัวร์โค้ด"] },
  { command: "mneme risk", triggers: ["riskiest part", "what should i be careful with", "risk hotspots", "where is the risk", "most dangerous code", "what to guard first", "biggest risk in the repo", "จุดเสี่ยงสุด", "ต้องระวังตรงไหน", "อันตรายสุดตรงไหน", "ระวังตรงไหนก่อน"] },
  { command: "mneme commit-check", triggers: ["does my commit message match", "is this commit mislabeled", "intent vs impact", "scope creep in this commit", "commit honesty", "commit message ตรงกับการแก้ไหม", "commit ป้ายผิดไหม"] },
  { command: "mneme authz", triggers: ["authz gap", "unauthenticated write", "missing auth check", "security holes", "endpoints without auth", "authorization audit", "unguarded endpoint", "ช่องโหว่ auth", "endpoint ไหนไม่มี auth", "ตรวจ security", "ตรวจสิทธิ์"] },
  { command: "mneme testgap", triggers: ["whats untested", "what is untested", "test gap", "untested critical", "where to add tests", "where should i add tests", "add tests", "is this tested", "am i editing untested code", "ยังไม่มี test", "ยังไม่มี test ตรงไหน", "test ขาดตรงไหน", "ตรงไหนยังไม่เทส", "ควรเขียน test ตรงไหน"] },
  { command: "mneme collision", triggers: ["will these agents collide", "do these branches conflict", "cross agent collision", "concurrent edits safe", "multi agent conflict", "merge collision", "agent ชนกันไหม", "branch ชนกันไหม", "แก้พร้อมกันชนไหม"] },
