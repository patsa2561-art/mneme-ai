 ...
 * Idempotent: the same repo always returns the same key until the file is
 * deleted. Private key is written with mode 0600.
 */
export function getIssuerKeyPair(repoRoot: string): IssuerKeyPair {
  ...
  try {
    ...
    // Persist best-effort; an ephemeral key still works for this process.
  }
  return { privateKey, publicKey, publicKeyB64, fingerprint: fingerprintOf(publicKeyB64) };
}
