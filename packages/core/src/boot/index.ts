...
export const DECISION_TABLE: readonly DecisionRow[] = [
  ...
  { when: "BEFORE you edit a function, rename/drop a DB table, or change an API route — to see what ELSE breaks across layers — OR the user asks to SEE/visualize the project", tool: "mneme.graph.blast { name } · mneme.graph.mermaid { name }  ·  CLI: mneme graph blast/stats/mermaid/view <name>", why: "CROSS-LAYER GRAPH — now reads JS/TS · Python · Go · Rust · Ruby. Links 4 layers: 💼 BUSINESS (rules from PRD/markdown, prove-or-unknown anchoring) ↔ 🌐 API (endpoints from route files) ↔ ⚙ CODE (functions) ↔ 🗄 DATA (db tables from the Prisma/SQL schema). `blast` = the cross-layer blast radius (edit a function → which tables it writes + which endpoints reach it). `mermaid` = a flowchart string you render INLINE in the chat to SHOW the user. `view` = a self-contained offline interactive HTML — the 🛰 IMPACT RADAR (center=the change · sectors=layers · rings=blast hop-distance · animated sweep+pulse · click a node to re-center; no name = a project-overview GALAXY you click into; --style lanes for a tiered SVG), signed by fingerprint. `card` = a static PNG/SVG share card (1200×630). `pr` (mneme.graph.pr) = the cross-layer blast radius of a whole git diff/PR — which DB tables + API routes + business rules the change set touches (a GitHub Action posts it on every PR). `check` (mneme.graph.check) = BEFORE applying a multi-file edit, flag a DB table/route/rule the diff touches that the user did NOT mention → route to the human. `reverse` (mneme.graph.reverse) = DROP SAFETY: before removing a table/endpoint, everything that depends on it + SAFE/RISKY/CRITICAL. `health` (mneme.graph.health) = cross-layer KEYSTONES (sole-writer single-points-of-failure) + ORPHANS (dead code); SCOPE COVENANT (world-first): mneme.scope.verify {intent, allow, diff} → HONORED/BREACHED signed verdict that an autonomous edit stayed within the architectural scope it DECLARED (the unpromised file/table/endpoint it reached); mneme.scope.fidelity = each agent's cross-vendor scope-keeping track record (Wilson-LB). GRAPHQL BREAKING: mneme.graphql.breaking {base} → diff GraphQL SDL surface (Query/Mutation/Subscription); removed-op or retyped-return = BREAKING (clients break). API BREAKING-CHANGE: mneme.api.breaking {base} → which endpoints removed/added vs a base ref, + which REMOVED ones are still CONSUMED (BREAKING, names the consumer) vs POSSIBLY breaking; cross-repo break git cant see. MEASURED ACCURACY: mneme.accuracy.report → real precision/recall/F1 per extractor on a labeled corpus (proves accuracy, not claims it; deterministic≠accurate). PROVABLE CROSS-LAYER: mneme.graph.prove {kind:drop|reaches} → extracts facts from the graph + the logic engine PROVES unsafe_to_drop(table) with a proof chain to the cited blocker (or LIKELY_SAFE=no structural blocker), or proves an endpoint reaches a table; heuristic→PROOF. THE LOGIC ENGINE: mneme.logic.check {facts,rules,mutexes,goal} → is a reasoning chain SOUND (not are facts true): PROVEN+proof-tree / CONTRADICTED (premises entail mutually-exclusive pair) / UNKNOWN (does not follow, never false). For checking an AI agents own argument before acting. CROSS-SERVICE: mneme.services.graph → across MANY repos/services, which service calls whose endpoints (producer↔consumer) + DANGLING calls to an endpoint no service produces; org-scale blast radius git cant see. ARCHITECTURAL INVARIANTS: mneme.invariants.check {mine?:true} → MINE the invariants the repo already upholds (zero-config, proven-at-mine-time) OR prove declared rules; HOLDS/VIOLATED+counterexample. CHANGE GATE: mneme.change.gate {baseline} → the ONE call before commit: fuses scar-vaccine + firewall → PASS/WARN/BLOCK. BEHAVIORAL DIFF: mneme.bdiff {baseline} → classify changed fns by TEXT×BEHAVIOR; surface NOISE (big text/no behavior change) + SILENT-RISK (tiny text/behavior changed); review only what matters. AUTO-EQUIV: mneme.equiv.diff {baseline} → extract OLD+NEW of each changed fn from git automatically + differential-test (no paste); which refactors preserved behavior, which broke. EQUIVALENCE RECEIPT: mneme.equiv.check {oldFn,newFn,args} → prove a PURE-fn refactor did not change behavior (differential boundary test → signed receipt or counterexample). TRUST GATEWAY: mneme trust-serve → run the trust suite (change-gate/scar/firewall/equiv/agent-rep) as a stateless HTTP service any agent POSTs to (AI-native SaaS front door). AGENT REPUTATION: mneme.agent.reputation {maxCommits} → who ships code that LASTS (Wilson-LB survival rate from git revert facts; durability not honesty). SCAR MINING: mneme.scar.mine {maxCommits} → derive the org scar corpus from repo history (fix/revert→mistake pairs) to .mneme/scars.mined.json for review (data flywheel). SCAR VACCINE: mneme.scar.check {code,files} → inject the org past-mistake lesson BEFORE you commit; fires on the mistake-shape, quiet when the fix(antibody) is present. ARCH FIREWALL: mneme.arch.firewall {baseline} → the gate for AI-generated change: PASS/WARN/BLOCK on whether the current code violates a load-bearing contract (severity weighted by contract age; declared policies honoured). On BLOCK do NOT commit. FAIR-DIVISION MEDIATOR: mneme.mediate {parties} → envy-free/equitable/Pareto split (2 parties via Adjusted Winner) + re-checkable neutrality proof; for divorce/inheritance/M&A/partnership. SECURITY POSTURE: mneme.security.audit → AppSec triad in one call (authz+exfil+injection) → CLEAR/REVIEW/AT-RISK + posture 0-100. INJECTION PATH: mneme.arch.injection → endpoint-reachable fn feeds request input into a dangerous sink (raw SQL/exec/eval/fs) with NO sanitizer (SQLi/cmd-inj/SSRF). EXFIL PATH: mneme.arch.exfil → endpoints that READ a sensitive table + reach the response with NO redaction (data-leak path; read-side mirror of authz gap). DEAD-PATH DETECTOR: mneme.arch.dead_paths → architectural data-flow smells: write-only tables (written never read), read-only tables, functions reachable from no endpoint (dead-code candidates). CONTRACT MAP: mneme.arch.contract_map {since} → mine ALL contracts, rank by age=load-bearing (FOUNDATIONAL near-sacred → RECENT fragile); one repo-wide view of what to respect vs revise. INVARIANT LINEAGE: mneme.arch.lineage {invariant,since} → the AGE of a contract: the commit it was first established + held since (mirror of bisect); old=load-bearing near-sacred, young=fragile. ARCHITECTURAL REGRESSIONS: mneme.arch.regressions {since} → self-diagnosing capstone: mine contracts upheld at baseline, re-prove at HEAD, bisect each break to commit+author (read before touching a repo). CHANGE-COUPLING: mneme.arch.coupling {since} → HIDDEN dependencies the code conceals but git history reveals: files that change together with NO graph link (implicit deps) vs STRUCTURAL (graph explains). CROSS-LAYER HOTSPOTS: mneme.arch.hotspots {since} → ranked refactor targets = files that churn (git) AND are cross-layer entangled (graph); sharper than churn×complexity. ARCHITECTURAL DECAY: mneme.arch.decay {since,samples} → is the architecture getting MORE entangled over time (ERODING/STABLE/IMPROVING) + coupling velocity + worst-step commit; structural-debt time-series. ARCHITECTURAL BISECT: mneme.arch.bisect {invariant,since} → git-bisect for a contract: the exact commit+author where an architectural invariant first broke (replays graph across history). ARCHITECTURAL-INVARIANTS: mneme.invariants.check → prove team-declared rules (table single-writer/private/guarded · endpoint exists) → HOLDS/VIOLATED+counterexample, design-by-contract for architecture. ARCHITECTURE LOCK: mneme.arch.check → package-lock for accountability; CI gate fails on a REGRESSION vs the committed contract (removed endpoint=BREAKING, new authz gap=SECURITY); additions fine; re-lock=reviewed act. THE FRONT DOOR — mneme.review (CLI `mneme review` [--base ref]) → one Codebase Accountability Report: graph + risk hotspots + authz gaps + untested keystones (the whole cross-layer suite in one call). ONBOARDING PATH: mneme.onboard.path → orient in an unfamiliar repo fast — real data-flows (entry→handler→callees→tables+rules), sensitive first. RISK HOTSPOTS: mneme.risk.hotspots → the ONE ranked list of the riskiest cross-layer nodes (keystone × untested × sensitive × authz-gap, fused) — the single what-to-guard-first answer. BLAST-AWARE COMMIT MSG: mneme.commit.suggest → generate an honest commit message from the diff impact. INTENT-vs-IMPACT: mneme.commit.check {message,diff} → a trivial-sounding commit (chore / fix-typo) that secretly rewrites a keystone/touches unmentioned tables = MISMATCH (mislabel/scope-creep). CROSS-LAYER AUTHZ GAP: mneme.authz.scan → endpoints whose handler reaches a WRITE to a sensitive table (accounts/payments) with NO auth function on the path (security smell to review first, may be middleware). CRITICAL UNTESTED SURFACE: mneme.testgap.scan → keystones/tables/endpoints NO test mentions (line-coverage-hidden risk); pass a diff → does THIS change reach untested critical surface. CROSS-AGENT COLLISION (world-first): mneme.collision.detect {changeSets:[{agent,diff}]} → where 2 concurrent agents/branches CONVERGE on the same table/endpoint/function across DIFFERENT files (the conflict git is blind to) + severity HIGH/MEDIUM/LOW. `benchmark` = measured extractor precision/recall; `drift --base <ref>` = architectural drift (cross-layer couplings that appeared/disappeared since a commit). The hosted xray server also serves it: GET /api/radar?gitUrl=<public repo> renders the radar for ANY public repo with no install. Deterministic, no LLM — every node/edge derives from a real file (nothing hallucinated); business→code links are anchored only on an explicit annotation or a strong name match, else the rule stays ORPHAN/UNKNOWN (never a guessed link). ★HONEST: reachable COUPLING to inspect, not a proven runtime break; bodies region-approximated, not a full AST" },
  { when: "you know what the user WANTS but not which of ~1000 tools fits (the agent's single front door)", tool: "mneme.morph { intent }  ·  CLI: mneme morph \"<intent>\"", why: "MORPH is the polymorphic plug: state intent in free language → it resolves the RIGHT capability and hands back the TYPED next-call (the exact MCP tool + projected args), decomposes a COMPOUND intent into an ordered plan[], and ABSTAINS (CLARIFY) rather than misfire (≥97.5% precision-when-it-routes, measured). Learn ONE tool, not 600. Falls back to mneme.gateway.route / mneme matrix search for ranked candidates" },
