...
export const DECISION_TABLE: readonly DecisionRow[] = [
  ...
  { when: "an agent operates DISCONNECTED from the cloud (Mars/space latency, a severed Starlink link, an air-gapped facility) and must self-govern, then PROVE the whole offline window stayed in bounds when the link returns", tool: "CLI: mneme aphelion open|act|seal|verify|merge · MCP: mneme.aphelion.capsule · core: aphelion.openSession/recordAction/sealCapsule/verifyCapsule/mergeCapsules", why: "APHELION — the brain at the farthest point from the cloud: a local CHARTER (mission/scope/forbidden/maxRisk) the agent self-gates every action against while off-grid → a tamper-evident hash-chained ledger → a sealed, signed capsule that proves charter-compliance for the ENTIRE disconnected window (a violation CANNOT be hidden after the fact) → a CRDT merge across the fleet on reconnect. The first governance layer that doesn't assume a cloud round-trip — built on Mneme's local-first, offline-verifiable core. ★HONEST: it proves what the agent RECORDED against its charter; it can't physically stop a disconnected action (that's autonomy) — it makes the record un-hideable" },
