...
export const DECISION_TABLE: readonly DecisionRow[] = [
  ...
  { when: "BEFORE installing/trusting an AI agent SKILL or MCP tool (from a marketplace, a repo, a teammate) — the #1 supply-chain risk in 2026", tool: "CLI: mneme skillscan <path> --sign", why: "an 8-point static scan (prompt-injection · exfiltration · secret · dangerous-command · obfuscation · external-fetch · credential-access · privilege-escalation, composing the firewall + MNEME-BC + egress) → SAFE/REVIEW/BLOCK + a content-hash + a NOTARY-signed provenance receipt any consumer verifies OFFLINE. ★HONEST: a static scan of declared content + a content-pinned attestation (raises the floor) — NOT a runtime-safety proof (a skill can fetch+run new code; pair with mneme heph at execution)" },
];
