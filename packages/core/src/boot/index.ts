...
export const DECISION_TABLE: readonly DecisionRow[] = [
  ...
  { when: "BEFORE you edit a function, rename/drop a DB table, or change an API route — to see what ELSE breaks across layers", tool: "mneme.graph.blast { name }  ·  CLI: mneme graph blast <name> / mneme graph stats", why: "CROSS-LAYER GRAPH — links CODE (functions) ↔ DATA (db tables from the Prisma/SQL schema) ↔ API (endpoints from route files) and returns the BLAST RADIUS across all three: edit a function → which tables it writes + which endpoints reach it. Deterministic, no LLM — every node/edge derives from a real file (nothing hallucinated). The cross-layer join a single-layer code-graph can't report. ★HONEST: reachable COUPLING to inspect, not a proven runtime break; bodies are region-approximated, not a full AST" },
