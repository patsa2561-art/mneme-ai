...
export const DECISION_TABLE: readonly DecisionRow[] = [
  ...
  { when: "BEFORE you edit a function, rename/drop a DB table, or change an API route — to see what ELSE breaks across layers — OR the user asks to SEE/visualize the project", tool: "mneme.graph.blast { name } · mneme.graph.mermaid { name }  ·  CLI: mneme graph blast/stats/mermaid/view <name>", why: "CROSS-LAYER GRAPH — links 4 layers: 💼 BUSINESS (rules from PRD/markdown, prove-or-unknown anchoring) ↔ 🌐 API (endpoints from route files) ↔ ⚙ CODE (functions) ↔ 🗄 DATA (db tables from the Prisma/SQL schema). `blast` = the cross-layer blast radius (edit a function → which tables it writes + which endpoints reach it). `mermaid` = a flowchart string you render INLINE in the chat to SHOW the user. `view` = a self-contained offline interactive HTML — the 🛰 IMPACT RADAR (center=the change · sectors=layers · rings=blast hop-distance · animated sweep+pulse · click a node to re-center; --style lanes for a tiered SVG), signed by fingerprint. Deterministic, no LLM — every node/edge derives from a real file (nothing hallucinated); business→code links are anchored only on an explicit annotation or a strong name match, else the rule stays ORPHAN/UNKNOWN (never a guessed link). ★HONEST: reachable COUPLING to inspect, not a proven runtime break; bodies region-approximated, not a full AST" },
