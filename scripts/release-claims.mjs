...
export const RELEASE_CLAIMS = {
  ...
  "2.19.64": {
    headline: "🦋 THE WASM CHRYSALIS + HOTFIX P1 18x cold-start regression + POST-PUBLISH SMOKE GATE — 13th world-first. THE ARCHITECTURAL ENDGAME: handles(WASM file on disk) = empty set post-instantiation. Native LoadLibrary opens kernel-level file section + lazy page-fault from disk forever -> EBUSY structurally unavoidable. WebAssembly.instantiate deserializes once into V8 heap -> disk file useless after that -> npm overwrite during execution is fine. Primitives shipped: loadAsBytes (synchronous read + immediate handle close + SHA-256) + instantiateFromBytes (wraps WebAssembly.instantiate; never throws) + verifyHandleClosed (openSync 'r+' probe + lsof; proves disk handle truly closed) + launchWasmFile (composed pipeline) + recordLaunch + readManifest + verifyLaunchChain (HMAC-chained ~/.mneme-global/launch-manifest.jsonl; 6th HMAC chain in Mneme composing with APOSTILLE). 24 deep tests including KILLER PROOFS: instance keeps working after .wasm file is overwritten/deleted/renamed. Plus HOTFIX: extractAndRedirect 2-layer fast-path (env-already-set short-circuit + size+mtime skip-copy) kills the 18x cold-start regression. Plus POST-PUBLISH SMOKE GATE workflow at .github/workflows/post-publish-smoke.yml (60s sleep for CDN + clean Ubuntu install + welcome/verify/trail + npm-deprecate-on-failure + auto-open GitHub issue). 4 new MCP tools (mneme.wasm.{launch, verify_handle, manifest, manifest_verify}). Total MCP tools 787 -> 791 (+4). 13th world-first: WASM-blob launcher + handle-closure invariant + cryptographic manifest as callable npm primitive. No AI tool worldwide ships any of this.",
    tools: [
      "mneme.wasm.launch",
      "mneme.wasm.verify_handle",
      "mneme.wasm.manifest",
      "mneme.wasm.manifest_verify",
    ],
  },
  "2.19.63": {
  ...
  "2.19.42": {
    ...
    tools: [
      "mneme.outcome.post_task",
      "mneme.outcome.submit_bid",
      "mneme.outcome.pick_winner",
      "mneme.outcome.score_outcome",
      "mneme.outcome.leaderboard",
      "mneme.zk_fairness.commit",
      "mneme.zk_fairness.generate_tests",
      "mneme.zk_fairness.verify",
      ...
      "mneme.honesty.strip_amendments",
    ],
  },
  "2.19.41": {
  ...
  "2.19.37": {
    ...
    tools: [
      "mneme.protocol.spec",
      "mneme.protocol.validate",
      "mneme.protocol.mint",
      "mneme.browser.detect",
      "mneme.browser.extract",
      "mneme.browser.mint",
      "mneme.citizens.aggregate",
      "mneme.citizens.report",
      "mneme.card.build",
      "mneme.card.render",
      "mneme.mayor.vote",
      "mneme.mayor.tally",
  ...
  "2.19.34": {
    ...
    tools: [
      ...
      "mneme.apostille.append",
      "mneme.apostille.verify_ledger",
      "mneme.apostille.query",
      "mneme.apostille.binder",
      "mneme.market.post_task",
      "mneme.market.submit_bid",
      "mneme.market.pick_winner",
      "mneme.market.score_outcome",
      "mneme.market.leaderboard",
      "mneme.fairness.commit",
      "mneme.fairness.generate_tests",
      "mneme.fairness.verify",
      "mneme.fairness.mint_cert",
      "mneme.fairness.audit_cert",
      "mneme.eternity.mint",
      "mneme.eternity.pin",
      "mneme.eternity.survival_score",
      "mneme.eternity.survival_cert",
      "mneme.eternity.resolve",
    ],
  },
  "2.19.33": {
