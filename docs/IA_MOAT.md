...
## R&D check — has anyone shipped this?

We searched the landscape before claiming "world-first." Honest findings:
...
**Nothing in the public landscape ships all four layers together** with a single fabric (SUPER NOVA WRAPPER), local-first guarantees, HMAC-signed audit, and cross-vendor reach. The closest is Letta + LangSmith bolted together, which still misses the witness fabric, scar/refuse layer, and the shared experience pool across agents.

If a competitor ships this within 12 months, we should still have the first-mover position in the open-source AI-tooling community (mneme-ai is on npm + GitHub, public since 2024).
