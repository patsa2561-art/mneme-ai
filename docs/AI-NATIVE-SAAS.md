...
- **HTTP Trust Gateway** — `mneme trust-serve` runs the whole suite as a stateless HTTP service any agent (any vendor, any machine, no install, no MCP) can POST to: `POST /change-gate` · `/scar/check` · `/firewall/check` · `/equiv` · `/agent-rep` → a JSON verdict. The agent sends the change; the service returns PASS / WARN / BLOCK + reasons. This is the front door — deploy one instance and every agent in your fleet hits the same gate. **Live demo: <https://trust.mneme-ai.space>** (open in a browser to try it; agents POST the endpoints). Deploy your own with `scripts/deploy-trust.sh` (additive + reversible).
